...
learning_logs = json.load(open("out/ll_em_uniform_125m.json"))
...
for k, v in pl_1_logprobs.items():
    for x in v:
        df.append({"em": k, "logprob": x})
    pl_1_logprobs[k] = (np.mean(v), min(np.std(v), 10))
df = pd.DataFrame(df)
...
sns.relplot(
    data=df, kind="line", x="em", y="logprob", ci="sd", marker="o", label="best logprob"
)
